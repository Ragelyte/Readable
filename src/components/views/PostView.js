...
class PostView extends Component {
    ...
    render() {
        return (
            <div>
                {this.props.post === undefined ? 'loading' :
                    ...
                    <div className="column is-half is-offset-one-quarter">
                        <div className="box">
                            <div className="content">
                                ...
                                {
                                    this.state.editPostFormOpen &&
                                    <a onClick={this.editPost} className="button is-light is-pulled-right is-small">Save</a>
                                }


                                <p>{this.props.post.category} &#8226; {moment(this.props.post.timestamp).format("MMMM Do YYYY")} &#8226; {this.props.post.author}</p>
                            </div>

                            <div className="column">
                                <div className="level-left">
                                    <div className="level-item">
                                        <span className="icon is-small"><i className="fas fa-heart"></i>{this.props.post.voteScore}</span>
                                    </div>
                                </div>
                            </div>
...
function mapStateToProps(state, ownProps) {
    return {
        post: state.posts.posts.find(post => post.id === ownProps.match.params.postId),
    }
}
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        editPost: (commentId, title, body) => dispatch(editPost(commentId, title, body))
    }
}
